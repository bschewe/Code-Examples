...
if ( ! isset($_SESSION['email']) ) {
	...
	if (isset($_SESSION['email']) ){
		$stmt = $pdo->prepare('SELECT first_name, last_name, headline, summary, profile_id, user_id FROM profile');
		$stmt->execute(array() );
		$profile = array();
...
<?php
	...
	echo("<tr><td>");
	echo('<a href="view.php?profile_id='.htmlentities($profiles['profile_id']).'">aS</a> / ');
    ...
    echo($profiles['summary']);
   	echo("</td><td>");
    echo('<a href="edit.php?profile_id='.htmlentities($profiles['profile_id']).'">Edit</a> / ');
